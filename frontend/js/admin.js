...
async function tryAutoLogin() {
  ...
  try {
    const r = await fetch(`${API_URL}/api/admin/sources`, { headers: authHeaders() });
    if (r.ok) enterAdmin();
  } catch {}
}
...
document.getElementById('uploadPdfBtn').addEventListener('click', async () => {
  ...
  try {
    const r = await fetch(`${API_URL}/api/admin/upload-pdf`, {
      method: 'POST',
      headers: authHeaders(),
      body: formData,
    });
...
document.getElementById('addLinkBtn').addEventListener('click', async () => {
  ...
  try {
    const r = await fetch(`${API_URL}/api/admin/add-link`, {
      method: 'POST',
...
async function reindexSource(name) {
  ...
  try {
    const r = await fetch(`${API_URL}/api/admin/reindex/${encodeURIComponent(name)}`, {
      method: 'POST',
      headers: authHeaders(),
    });
    const d = await r.json();
...
async function deleteSource(name) {
  ...
  try {
    const r = await fetch(`${API_URL}/api/admin/sources/${encodeURIComponent(name)}`, {
      method: 'DELETE',
      headers: authHeaders(),
    });
    const d = await r.json();
